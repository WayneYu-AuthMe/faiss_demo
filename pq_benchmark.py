...
def product_quant_benchmark(k: int, gt: list, param: str, output_csv_path: str):
    pq_index_timer = acv.Timer()
    with pq_index_timer:
        index = faiss.index_factory(d, param, faiss.METRIC_INNER_PRODUCT)
        index.ntotal
        faiss.normalize_L2(xb)
    print('K-means is trained:', index.is_trained)  # Shoud be False for training k-means
    pq_train_timer = acv.Timer()
    with pq_train_timer:
        index.train(xb)
    index.add(xb)
    faiss.normalize_L2(xq)
    pq_search_timer = acv.Timer()
    with pq_search_timer:
        res_distance, res_index = index.search(xq, k)
    ...
    result_df = pd.DataFrame(columns=['query_res', 'gt', 'label'])
...
if __name__ == '__main__':
    ...
    # output_csv_path = 'pq32_result.csv'
    # param = 'PQ32'
    # product_quant_benchmark(k, gt, param, output_csv_path)
    ...
    output_csv_path = 'ivfpq32_result.csv'
    param = 'IVF100,PQ32'
    product_quant_benchmark(k, gt, param, output_csv_path)
    result = pd.read_csv('ivfpq_result.csv')
    scores = result['cos_sim'].to_list()
    labels = result['label'].to_list()
    print('Accuracy:', len(result[result.label == 1.0]) / len(result))
    print('Number of Wrong:', len(result[result.label == 0.0]))
