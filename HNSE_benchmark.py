...
def HNSW_benchmark(k: int, gt: list, param: str, output_csv_path: str):
    # HNSW
    HNSW_index_timer = acv.Timer()
    with HNSW_index_timer:
        index = faiss.index_factory(d, param, faiss.METRIC_INNER_PRODUCT)
        print('HNSW_benchmark trained:', index.is_trained)   # Shuld be True
        index.ntotal
        faiss.normalize_L2(xb)
        index.add(xb)
        faiss.normalize_L2(xq)
    HNSW_search_timer = acv.Timer()
    with HNSW_search_timer:
...
if __name__ == '__main__':
    ...
    output_csv_path = 'HNSW64_result.csv'
    param = 'HNSW64'
    HNSW_benchmark(k, gt, param, output_csv_path)
